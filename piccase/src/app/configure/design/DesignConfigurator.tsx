...
const DesignConfigurator = ({ configId, imageUrl, imageDimensions }: DesignConfiguratorProps) => {
  ...
  const [options, setOptions] = useState<{
    ...
    acabamento: (typeof FINISHES.options)[number]
  }>({
    color: COLORS[0],
    model: MODELS.options[0],
    material: MATERIALS.options[0],
    acabamento: FINISHES.options[0],
  })
  ...
  async function saveConfiguration() {
    try {
      ...
      toast({
        title: 'Algo deu errado',
        description: 'Houve um problema ao salvar sua configuração, por favor, tente novamente.',
        variant: 'destructive',
      })
    }
  }
          ...
          <div className='w-full h-full flex justify-end items-center'>
            <div className='w-full flex gap-6 items-center'>
              <p className='font-medium whitespace-nowrap'>
                {formatPrice(
                  (BASE_PRICE + options.acabamento.price + options.material.price) /
                  100
                )}
              </p>
              <Button
